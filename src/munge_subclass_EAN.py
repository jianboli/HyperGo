""" This file process the new data esprite_ord_graph.csv and changed back to khk_ean"""
import pickle
import pandas as pd
import numpy as np
...
ret_rate = data.groupby("n_subclass_code").agg(cal_adj_ret_rate)
...
def ret_item_collection(g):
    return [list(g.loc[g['RET_Items']>0, "n_subclass_code"])]

bsk_ret_item_collection = data.groupby("OrderNo").agg(ret_item_collection)
bsk_ret_item_collection = bsk_ret_item_collection[['n_subclass_code']]
...
# num_item_bsk = data.groupby(["OrderNo", "SubClassCode"]).agg({'khk_ean': 'count'})\
#     .groupby(level='OrderNo').agg({'khk_ean': 'max'})
# multi_item_bsk = num_item_bsk.query('khk_ean > 1')
# multi_item_bsk_ret_rate = pd.merge(bsk_ret_label, multi_item_bsk, left_index=True, right_index=True, how='inner')
#
...
data['n_subclass_code'] = data['n_subclass_code'].astype('category')
data['OrderNoIdx'] = data['OrderNo'].cat.codes
data['n_subclass_codeIdx'] = data['n_subclass_code'].cat.codes
data['RET_Items'] = np.sign(data['RET_Items'].values)
bsk_item_pair = data.groupby(['OrderNoIdx', 'n_subclass_codeIdx']).agg({'OUT_Items': 'count', 'RET_Items': 'sum'})


h_mat = sparse.coo_matrix((bsk_item_pair['OUT_Items'].values,
                          (bsk_item_pair.index.get_level_values("OrderNoIdx").values,
                           bsk_item_pair.index.get_level_values("n_subclass_codeIdx").values)))
h_mat = h_mat.tocsr()
...
style_color = list(data['n_subclass_code'].cat.categories.values)
